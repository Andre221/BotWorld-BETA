...
module.exports.run = function (command, args, message, bot) {
    request.get('https://opentdb.com/api.php?amount=1&type=multiple', (error, response, body) => {
        ...
        body.results[0].incorrect_answers = body.results[0].incorrect_answers.map(v=>decode(v));
        body.results[0].correct_answer = decode(body.results[0].correct_answer)
        let answers = shuffle(body.results[0].incorrect_answers).concat([body.results[0].correct_answer]);
        let aText = '';
